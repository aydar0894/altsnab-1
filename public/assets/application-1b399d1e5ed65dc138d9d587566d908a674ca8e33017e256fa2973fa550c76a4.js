...
(function() {
  $(function() {
    var previous_value;
    $(document).on('cocoon:after-remove', '.subitems', function(e, elem) {
      var new_total_items_sum, subitem_price, subitems, subitems_price_display, subitems_price_total, total_items_sum;
      subitem_price = $(elem).find('option[selected="selected"]').data('price');
      subitems = $(elem).closest('.product').find('.subitems');
      subitems_price_total = subitems.data('price-total') - subitem_price;
      ...
      new_total_items_sum = total_items_sum - subitem_price;
      $('#total_items_sum').data('total', new_total_items_sum);
      return $('#total_items_sum span').text(new_total_items_sum);
    });
    previous_value = 0;
    $(document).on('focus', '.subitems select', function(e) {
      return previous_value = parseInt($(this).find(':selected').attr('data-price'));
    });
    return $(document).on('change', '.subitems select', function(e) {
      var new_value, subitems, subitems_price_display, subitems_price_total;
      ...
      console.log(subitems_price_total, new_value - previous_value);
      subitems.data('price-total', subitems_price_total);
      subitems_price_display = $(this).closest('.product').find('.subitems-price span');
      return subitems_price_display.text(subitems_price_total);
    });
  });
...
(function() {
  $(function() {
    return $(document).on('change', '#item_category_id', function() {
      var callback, category_id, item_id;
      category_id = $(this).find(":selected").attr("value");
      item_id = $('#item-id-input').attr("value");
      callback = function(response) {
        $('#fields').empty();
        return $.each(response, function(index, item) {
          return $('#fields').append(item.name + ' <input type="text" name="item_field[' + item.cf_id + ']" value="' + item.value + '"/><br>');
        });
      };
      return $.get('/item_category_fields/' + item_id + '/' + category_id + '.json', callback, 'json');
    });
  });
...
(function() {
  $(function() {
    return $(document).on('ajax:success', '.cart_item_increment, .cart_item_decrement', function() {
      var cart_item, count, new_count, new_total, new_total_items_sum, price, sign, total_items_sum;
      sign = 1;
      ...
      return cart_item.find('.total span').text(new_total);
    });
  });
